...
class SmartPromptGenerator:
    ...
    def __init__(self):
        ...
        self.available_models = self.get_available_gemini_models()
    ...
    def get_gemini_api_key(self):
        try:
            ...
            print("Warning: Gemini API key not found, will use basic prompt enhancement")
            return ""
    
    def get_available_gemini_models(self):
        """Get available Gemini models for the dropdown"""
        try:
            # Try to import from list_models.py
            from .list_models import get_gemini_models
            available_models = get_gemini_models()
            # Filter for models that are good for text enhancement
            text_models = [m for m in available_models if any(x in m for x in ["gemini-1.5", "gemini-2.0", "gemini-pro"])]
            if not text_models:
                text_models = ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-2.0-flash", "gemini-2.0-pro"]
            return text_models
    ...
    def INPUT_TYPES(cls):
        ...
        inputs = {
            "required": {
                ...
                "randomize_seed": ("INT", {"default": 0, "min": 0, "max": 999999999, "step": 1}),
                "enhance_with_ai": ("BOOLEAN", {"default": True}),
                "preserve_user_text": ("BOOLEAN", {"default": True}),
                "gemini_model": (instance.available_models, {"default": "gemini-1.5-flash"}),
    ...
    RETURN_NAMES = ("enhanced_prompt", "negative_prompt", "original_prompt", "used_model")
    ...
    def generate_random_base_prompt(self):
        ...
        # Set a new random seed for truly random results each time
        random.seed()
    ...
    def enhance_with_gemini(self, prompt, negative_prompt, gemini_model, creativity_level, focus_on, applied_styles, subject_type, preserve_user_text):
        ...
        if not self.gemini_api_key:
            return prompt, negative_prompt, "No API key"
        ...
        try:
            ...
            - NEVER include any negative elements in the positive prompt
            
            FOR THE NEGATIVE PROMPT:
            - Create a separate negative prompt only with elements to avoid
            - Focus on technical issues like blurry, pixelated, deformed that should be avoided  
            - Keep the negative prompt concise and relevant
            - Do NOT mix positive and negative elements
            """
            ...
            # Negative prompt enhancement as a separate call
            input_negative = f"{system_prompt}\n\nMAIN PROMPT: {prompt}\n\nCreate a NEGATIVE PROMPT listing only elements to avoid:"
            ...
            if preserve_user_text and not enhanced_prompt.startswith(prompt):
                if "," in prompt:
                    first_part = prompt.split(",")[0]
                    if not enhanced_prompt.startswith(first_part):
    ...
    def generate_fully_random_prompt(self, gemini_model):
        """Generate a completely random prompt using Gemini API"""
        if not self.gemini_api_key:
            # Fallback to basic random prompt if no API key
            return self.generate_random_base_prompt(), "", "No API key"
    ...
    def generate_smart_prompt(self, base_prompt, negative_prompt, random_mode, num_random_styles, 
                             randomize_seed, enhance_with_ai, preserve_user_text, gemini_model,
                             creativity_level, focus_on, subject_type="Auto-detect", 
                             prompt_template="", **kwargs):
        """Generate a smart prompt by combining styles and enhancing with AI"""
        
        # Set custom seed if provided, otherwise use random
        if randomize_seed > 0:
        ...
        if enhance_with_ai and self.gemini_api_key:
            ...
            used_model = "No enhancement applied"
        
        return (final_prompt, final_negative, original_prompt, used_model)
